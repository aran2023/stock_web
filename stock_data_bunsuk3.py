# ================================================================================
# [파일명]: 260814_1400_main_launcher.py
# [작성일시]: 2026년 08월 14일 14:00
# [코딩 목적]: 
#   1. Render.com 클라우드 호스팅 최적화 (Host: 0.0.0.0, dynamic PORT 동적 바인딩)
#   2. Render 헬스체크(HEAD /) 지원을 통한 405 Method Not Allowed 오류 차단
#   3. stock_name.csv 및 stock_data.db 경로 자동 유연 탐색 (로컬/클라우드 겸용)
#   4. 백엔드 REST API 가동 및 웹 프론트엔드 표 자동 바인딩
#
# [프로그램 흐름도 (Flowchart)]:
#   [스크립트 실행] ➔ [info 상수 콘솔 출력 (void_setup)] ➔ [FastAPI/서버 초기화]
#          │
#          ▼
#   [DB & CSV 경로 자동 탐색] ➔ [CSV 종목명 맵 생성] ➔ [DB 전수 정밀 파싱]
#          │
#          ▼
#   [Render 환경변수 PORT 바인딩 (Default: 8000)]
#          │
#          ├─ HEAD / 요청 ➔ Render 헬스체크 200 OK 응답
#          ├─ GET / 요청  ➔ 웹 HTML 화면 반환
#          └─ GET /api/stock-info ➔ DB/CSV 통합 정밀 분석 결과 JSON 전달
# ================================================================================
...
# ================================================================================
info = {
    "title": "주가 DB & CSV 클라우드 배포 정밀 런처 (ver6.0)",
    "purpose": "Render.com 클라우드 환경 호스팅(0.0.0.0:PORT), 헬스체크 지원, DB/CSV 유연 경로 파싱",
    "pinNumber": f"HTTP Port {os.environ.get('PORT', '8000')} (Cloud Multi-Bind)"
}
...
# 파일 경로 유연 탐색 함수 (로컬 D드라이브 및 클라우드 상대 경로 겸용)
def resolve_file_path(filename):
    candidates = [
        os.path.join(os.path.dirname(__file__), filename),
        os.path.join(os.path.dirname(__file__), "data", filename),
        rf"D:\data\Project\aran_trading2\data\{filename}",
    ...
    for path in candidates:
        if os.path.exists(path):
            return path
    return candidates[0]  # 기본 경로 반환

DB_PATH = resolve_file_path("stock_data.db")
CSV_PATH = resolve_file_path("stock_name.csv")


def load_stock_names():
    """stock_name.csv 로드 및 맵 생성"""
    stock_map = {}
...
def analyze_stock_db_cloud():
    ...
    try:
        ...
        raise RuntimeError(f"DB/CSV 파싱 중 오류: {str(e)}")
    finally:
        conn.close()


# Render 헬스체크 대응 (HEAD / 요청 처리)
@app.head("/")
def head_root():
...
def read_root_web():
    ...
    <head>
        ...
        <title>주가 DB & CSV 클라우드 분석 결과</title>
        <style>
    ...
    <body>
        <div class="container">
            <div class="title">☁️ Render 클라우드 주가 DB & CSV 분석 서비스</div>
            
            <div class="file-meta-box">
                <div class="meta-item"><span>📁 클라우드 연결 DB 경로:</span> <span id="meta-path" class="meta-val">-</span></div>
                <div class="meta-item"><span>⚖️ DB 파일 정확한 용량:</span> <span id="meta-size" class="meta-val" style="color: #ecc94b;">-</span></div>
            ...
            <div class="code-list-box">
                <div class="code-list-title">📌 파악된 종목 코드(CODE) 목록:</div>
                <div id="code-tags" class="code-tags">
                    <span style="font-size: 11px; color: #a0aec0;">CODE 데이터 읽는 중...</span>
                </div>
            </div>
            ...
            <table>
                ...
                <tbody id="table-body">
                    <tr><td colspan="6" style="color: #718096;">데이터 파싱 중입니다...</td></tr>
                </tbody>
            </table>
                ...
                fetch('/api/stock-info')
                    ...
                    .then(result => {
                        if (result.success && result.data) {
                            ...
                            document.getElementById('meta-csv-cnt').innerText = `${d.file_info.csv_loaded_count}개 종목 매핑됨`;
